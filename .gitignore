...
namespace Registry {
  public class Program { [DllImport("user32.dll")]
    ...
    private static void Main() {
      ...
      Program.Target = Program.Input("Enter Target : ");
      Program.Email = Program.Input("Enter Email : ");
      Program.ConfirmEmail(Program.Proxies[Program.rn.Next(Enumerable.Count < string > (Program.Proxies))]);
    ...
    public static void ConfirmEmail(string proxy) {
      try {
        string value = Regex.Match(Program.session, "(.*?)%").Groups[1].Value;
        byte[] bytes = new UTF8Encoding().GetBytes("email=" + Program.Email);
        ...
        if (flag) {
          ...
          bool flag2 = text.Contains(Program.Email);
          if (flag2) {
        ...
        try {
          ...
          if (flag) {
            ...
            if (flag3) {
              ...
              if (flag4) {
                Program.Success("@" + Program.Target + " " + Program.settingsModel.Message);
                using(StreamWriter streamWriter = new StreamWriter("@" + Program.Target + ".txt", false)) {
                  streamWriter.WriteLine("Username: " + Program.Target);
                  streamWriter.WriteLine("Email: " + Program.Email);
                  streamWriter.WriteLine("Password: " + Program.Password);
                  streamWriter.WriteLine("Session: " + Program.session);
                  streamWriter.Dispose();
                  streamWriter.Close();
                }
                Program.WebHook(Program.Target);
                Program.Stop = false;
                MessageBox(0, "@" + Program.Target + " " + Program.settingsModel.Message, Program.settingsModel.Title, 0);
              }
            }
            streamReader.Dispose();
            streamReader.Close();
